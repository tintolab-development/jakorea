...
const PLATFORM_JUSO_MISSING_KEY_MESSAGE =
...
const PLATFORM_NEIS_MISSING_KEY_MESSAGE =
  'NEIS API 키가 설정되지 않았습니다. apps/platform/.env에 VITE_NEIS_API_KEY를 설정한 뒤 Platform dev server를 재시작해 주세요.'

const PLATFORM_CAREER_NET_MISSING_KEY_MESSAGE =
  '커리어넷 API 키가 설정되지 않았습니다. apps/platform/.env에 VITE_CAREER_NET_API_KEY(또는 VITE_CAREEAR_NET_API_KEY)를 설정한 뒤 Platform dev server를 재시작해 주세요.'

export function getPlatformJusoMissingKeyMessage(): string {
  return PLATFORM_JUSO_MISSING_KEY_MESSAGE
}

export function getPlatformNeisMissingKeyMessage(): string {
  return PLATFORM_NEIS_MISSING_KEY_MESSAGE
}

export function getPlatformCareerNetMissingKeyMessage(): string {
  return PLATFORM_CAREER_NET_MISSING_KEY_MESSAGE
}
