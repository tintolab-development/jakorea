/**
 * 첨부 파일 다운로드.
 * mock 등 실파일이 없을 때도 파일명의 확장자와 MIME을 유지한 빈 파일로 저장한다.
 */
...
/**
 * 첨부 다운로드. 실 URL이 있으면 해당 리소스를 파일명으로 저장하고,
 * 없으면 확장자에 맞는 MIME의 빈 Blob을 같은 파일명으로 저장한다.
 */
export function downloadAttachment(fileName: string, url?: string): void {
