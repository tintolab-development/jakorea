...
import styles from './pf-item-delete-button.module.css'

export type PFItemDeleteButtonProps = Omit<
...
/** 항목 행 우측 원형 X — CMS 템플릿 ItemDeleteButton과 동일 스펙 */
export function PFItemDeleteButton({
  className,
  'aria-label': ariaLabel,
  onClick,
  disabled = false,
  ...rest
}: PFItemDeleteButtonProps) {
  const reactId = useId()
  const maskId = `pf-item-delete-mask-${reactId.replace(/:/g, '')}`
