...
// 프로그램 mock 시드 제거(실제 API) — 파생 일정 mock도 비움
const hasProgramSeeds = mockPrograms.length > 0

// Phase 0.2.6: 일반 일정 생성 (30개)
const baseSchedules: Schedule[] = !hasProgramSeeds
  ? []
...
const instructor1Schedules: Schedule[] = !hasProgramSeeds
  ? []
...
// 오늘(27일) 기준 일정 10개
const todaySchedules: Schedule[] = !hasProgramSeeds
  ? []
  : Array.from({ length: 10 }, (_, index) => {
      const programIndex = Math.floor(Math.random() * mockPrograms.length)
      const program = mockPrograms[programIndex]
...
// 내일(28일) 기준 일정 10개
const tomorrowSchedules: Schedule[] = !hasProgramSeeds
  ? []
  : Array.from({ length: 10 }, (_, index) => {
      const programIndex = Math.floor(Math.random() * mockPrograms.length)
      const program = mockPrograms[programIndex]
...
// 내일(28일) 기준 추가 일정 10개
const tomorrowSchedulesAdditional: Schedule[] = !hasProgramSeeds
  ? []
  : Array.from({ length: 10 }, (_, index) => {
      const programIndex = Math.floor(Math.random() * mockPrograms.length)
      const program = mockPrograms[programIndex]
...
export function buildCompanySchoolSchedulesForVisibleRange(
  ...
  _allowedCompanySchoolProgramIdSet: Set<string> | null
): Schedule[] {
  void _visibleDateKeys
  void _allowedCompanySchoolProgramIdSet
  return []
}
