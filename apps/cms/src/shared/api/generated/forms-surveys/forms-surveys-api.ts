...
/**
 * ### 이 API가 하는 일
 * - POST /api/admin/form-responses/{responseId}/privacy-detail
 * - API 분류: 내부 처리 또는 보조 API
 * - 사용하는 화면: 화면 직접 호출보다는 운영/진단 또는 내부 처리에서 사용합니다.
 * - 호출 방식: `POST /api/admin/form-responses/{responseId}/privacy-detail`
 *
 * ### 화면/프론트 사용 기준
 * - 요청값 출처: Swagger 요청 폼 또는 화면 필터/선택값
 * - 응답 사용 위치: 응답 본문을 화면 상태와 조회 캐시에 반영
 ...
 * @summary POST /api/admin/form-responses/{responseId}/privacy-detail
 */
const revealDocument = (
 ...
 options?: SecondParameter<typeof customInstance<FormResponseDocumentSnapshotResponse>>,) => {
      ...
      {url: `/api/admin/form-responses/${responseId}/privacy-detail`, method: 'POST',
      headers: {'Content-Type': 'application/json', },
      data: revealRequest
    },
      options);
...
/**
 * ### 이 API가 하는 일
 * - 폼 응답 피드백 등록 (과제·교육계획서·교육일지)
 * - API 분류: 내부 처리 또는 보조 API
 * - 사용하는 화면: 화면 직접 호출보다는 운영/진단 또는 내부 처리에서 사용합니다.
 * - 호출 방식: `POST /api/admin/form-responses/{responseId}/feedback`
 *
 * ### 화면/프론트 사용 기준
 * - 요청값 출처: Swagger 요청 폼 또는 화면 필터/선택값
 * - 응답 사용 위치: 응답 본문을 화면 상태와 조회 캐시에 반영
 ...
 * ### 권한/보안
 * - 호출 가능 계정: 관리자 계정
 * - 필요 권한: FORM_RESPONSE_WRITE 권한 필요
 * - 접근 범위: 관리자 CMS 권한 범위
 * - 인증 API가 아니라면 Swagger 우측 상단 Authorize에 관리자 또는 회원 Bearer 토큰을 입력한 뒤 호출합니다.
 *
 * ### 개인정보/감사 정책
 ...
 * - 생성/수정/삭제 API 성공 후에는 관련 목록과 상세 조회를 다시 불러옵니다.
 * - 날짜, 금액, 상태 배지는 백엔드 원본 값과 화면 정의서의 라벨 매핑을 기준으로 표시합니다.
 * - 검토 메모: Wave11 auto-send feedback producer; inserts form_response_feedback and publishes WEB notice
 * @summary 폼 응답 피드백 등록 (과제·교육계획서·교육일지)
 */
const createFeedback = (
    responseId: number,
    formResponseFeedbackCreateRequest: FormResponseFeedbackCreateRequest,
 options?: SecondParameter<typeof customInstance<FormResponseFeedbackResponse>>,) => {
      return customInstance<FormResponseFeedbackResponse>(
      {url: `/api/admin/form-responses/${responseId}/feedback`, method: 'POST',
      headers: {'Content-Type': 'application/json', },
      data: formResponseFeedbackCreateRequest
    },
      options);
    }
...
export type RevealDocumentResult = NonNullable<Awaited<ReturnType<ReturnType<typeof getJAKoreaCMSBackendAPIFormsSurveysSubset>['revealDocument']>>>
export type CreateFeedbackResult = NonNullable<Awaited<ReturnType<ReturnType<typeof getJAKoreaCMSBackendAPIFormsSurveysSubset>['createFeedback']>>>
