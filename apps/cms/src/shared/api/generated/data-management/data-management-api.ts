...
import type {
  ...
  ApiResponseMaterialKitCurrentConfigResponse,
  ...
  MaterialKitCurrentConfigRequest,
  MaterialKitCurrentConfigResponse,
  MaterialKitRequest,
  MaterialKitResponse,
  ...
  SponsorLogoBulkDownloadRequest,
  SponsorRequest,
  SponsorResponse,
...
/**
 * ### 이 API가 하는 일
 * - 후원사 로고 일괄 Worker ZIP 생성
 * - API 분류: 내부 처리 또는 보조 API
 * - 사용하는 화면: 화면 직접 호출보다는 운영/진단 또는 내부 처리에서 사용합니다.
 * - 호출 방식: `POST /api/admin/sponsors/logo-download/jobs`
 *
 * ### 화면/프론트 사용 기준
 * - 요청값 출처: Swagger 요청 폼 또는 화면 필터/선택값
 * - 응답 사용 위치: 응답 본문을 화면 상태와 조회 캐시에 반영
 ...
 * - 생성/수정/삭제 API 성공 후에는 관련 목록과 상세 조회를 다시 불러옵니다.
 * - 날짜, 금액, 상태 배지는 백엔드 원본 값과 화면 정의서의 라벨 매핑을 기준으로 표시합니다.
 * - 검토 메모: Notion 후원사 상세 로고 일괄 다운로드 Worker ZIP
 * @summary 후원사 로고 일괄 Worker ZIP 생성
 */
const sponsorLogos = (
    sponsorLogoBulkDownloadRequest: SponsorLogoBulkDownloadRequest,
 ...
 * - API 분류: 내부 처리 또는 보조 API
 * - 사용하는 화면: 화면 직접 호출보다는 운영/진단 또는 내부 처리에서 사용합니다.
 * - 호출 방식: `DELETE /api/admin/sponsors/{sponsorId}/program-histories/{programId}`
 *
 * ### 화면/프론트 사용 기준
 * - 요청값 출처: Swagger 요청 폼 또는 화면 필터/선택값
 * - 응답 사용 위치: 응답 본문을 화면 상태와 조회 캐시에 반영
 ...
 * - 날짜, 금액, 상태 배지는 백엔드 원본 값과 화면 정의서의 라벨 매핑을 기준으로 표시합니다.
 * - 검토 메모: 2026-09-09 cumulative content/runtime/settlement final merge contract synchronization
 * @summary 후원사 프로그램 진행 이력 연결 삭제
 */
const deleteProgramHistory1 = (
    sponsorId: number,
    programId: number,
 options?: SecondParameter<typeof customInstance<ApiResponseDeleteResponse>>,) => {
      return customInstance<ApiResponseDeleteResponse>(
      {url: `/api/admin/sponsors/${sponsorId}/program-histories/${programId}`, method: 'DELETE'
    },
      options);
    }
...
export type List1Result = NonNullable<Awaited<ReturnType<ReturnType<typeof getJAKoreaCMSBackendAPIDataManagementSubset>['list1']>>>
...
export type Get2Result = NonNullable<Awaited<ReturnType<ReturnType<typeof getJAKoreaCMSBackendAPIDataManagementSubset>['get2']>>>
