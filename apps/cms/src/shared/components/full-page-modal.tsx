          ...
          <div className="full-page-modal__body-header">
            ...
            <div className="full-page-modal__actions">
              {templateTabType === 'issuance' ? (
                <CmsButton variant="default" size="medium" onClick={onDownloadDocument}>
                  문서 다운로드
                </CmsButton>
              ) : null}
              <CmsButton variant="default" size="medium" onClick={onPreview}>
                미리보기
              </CmsButton>
              <CmsButton variant="primary" size="medium" onClick={onSave}>
                저장
              </CmsButton>
            </div>
          </div>
...
export function TemplateFullpageModalCard({ children, className }: TemplateFullpageModalCardProps) {
  return (
    <section className={['full-page-modal-card', className].filter(Boolean).join(' ')}>
      {children}
    </section>
  )
}
...
export function TemplateFullpageModalCardDescription({
  ...
  return (
    <div className={['full-page-modal-card__description', className].filter(Boolean).join(' ')}>
      {children}
    </div>
  )
}
