...
import { normalizeSmsSendPhone } from '@/features/notifications/model/sms-send/recipients'
...
function rowsFromPhones(phones: string[]): DraftRow[] {
  const saved: DraftRow[] = phones.map(phone => ({
    key: nextKey(),
    mode: 'saved',
    phone: normalizeSmsSendPhone(phone),
  }))
  if (saved.length === 0) {
...
export function RecipientManualModal({
  ...
  function handleRegister(key: string) {
    ...
    const phone = normalizeSmsSendPhone(current.phone)
    ...
    const duplicate = rows.some(
      row => row.key !== key && row.mode === 'saved' && row.phone.replace(/\D/g, '') === digits
    )
    if (duplicate) {
  ...
  function handleCancelInput(key: string) {
    setRows(prev => {
  ...
  function handleConfirm() {
    ...
    if (pending && pending.mode === 'input') {
      const phone = normalizeSmsSendPhone(pending.phone)
      if (!isValidKoreanPhoneNumber(phone)) {
        showAlert({ title: '안내', content: '올바른 휴대폰 번호를 입력하세요.' })
        return
      }
