...
export function TemplatePreviewModal({
  ...
  }, [open, isA4DocumentPreviewLayout, focusedParagraphId, focusedPage1Based, safeActivePage, setSearchParams])
  ...
  useLayoutEffect(() => {
    if (!open || !isA4DocumentPreviewLayout) return
    if (!focusVisibleOnCurrentPage || focusedParagraphId == null || focusedParagraphId === '') return
    const root = previewBodyRef.current
    if (root == null) return
    const el = root.querySelector(`[data-paragraph-id="${CSS.escape(focusedParagraphId)}"]`)
    el?.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
  }, [open, isA4DocumentPreviewLayout, focusVisibleOnCurrentPage, focusedParagraphId, safeActivePage])
