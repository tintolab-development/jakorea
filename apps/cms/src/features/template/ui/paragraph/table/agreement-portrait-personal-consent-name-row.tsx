...
export function PortraitAffiliationBody({
  ...
  const inputPlaceholder =
    !placeholder.trim() || placeholder.trim() === '소속 기관명'
      ...
      : placeholder.trim()
  const committedAffiliation = noAffiliation ? '' : affiliation
  const {
    value: editAffiliation,
    setValue: setEditAffiliation,
    flush: flushAffiliation,
  } = useDeferredFieldCommit(
    committedAffiliation,
    interactive && !noAffiliation ? onChange : undefined
  )
    ...
    <div className="agreement-portrait-personal-consent-name-row__affiliation">
      <div className="agreement-portrait-personal-consent-name-row__affiliation-input-shell">
        <CmsInput
          ...
          value={noAffiliation ? '' : editAffiliation}
