...
import type { UjatSurveyScaleChartDatum } from '../lib/aggregate-survey-poll-results'
import './ujat-survey-poll-results.css'

type UjatSurveyScaleResultChartProps = {
  data: UjatSurveyScaleChartDatum[]
}

function ScaleChartTooltip({ active, payload }: TooltipProps<number, string>) {
  if (!active || payload == null || payload.length === 0) return null
  const datum = payload[0]?.payload as UjatSurveyScaleChartDatum | undefined
...
export function UjatSurveyScaleResultChart({ data }: UjatSurveyScaleResultChartProps) {
