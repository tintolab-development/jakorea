...
import jaLogoGrayUrl from './assets/ja-logo-gray.svg'
import facebookIconUrl from './assets/icon/facebook.svg'
import instagramIconUrl from './assets/icon/instagram.svg'
import youtubeIconUrl from './assets/icon/youtube.svg'
import ministryLogoUrl from './assets/logo/footer-logo-1.svg'
import taxLogoUrl from './assets/logo/footer-logo-2.svg'
import rightsLogoUrl from './assets/logo/footer-logo-3.svg'
...
export function PlatformPreviewFooter() {
  return (
    <footer className={`${styles.footer} platform-preview-footer`}>
      <div className={styles['navigation-area']}>
        <nav className={`${styles.navigation} platform-preview-footer__navigation`} aria-label="하단 메뉴">
          {footerNavItems.map(item => (
            <button className={styles['navigation-button']} type="button" key={item} tabIndex={-1}>
      ...
      <div className={`${styles['content-area']} platform-preview-footer__content`}>
        ...
        <div className={styles['social-area']}>
          <div className={styles['social-links']}>
            {socialLinks.map(({ label, iconUrl }) => (
              <span className={styles['social-link']} aria-label={label} key={label}>
