/**
 * 실적 관리 엑셀 시트2(합계)용 — 필터된 Program[] 를 피벗형 행 모델로 집계한다.
 */

import type { Program, TargetLevel } from '@/types/domain'

/** 시트 헤더(스크린샷 13열) */
export const PIVOT_SHEET_HEADERS = [
  '행 레이블',
  '개수: 학교명(기관)',
  '합계: 교육시간',
  '합계: 학급수',
  '합계: 남',
  '합계: 여',
  '합계: 총참가자',
  '합계: 일반자원봉사자',
  '합계: 임직원자원봉사자',
  '합계: 재참여자원봉사자',
  '합계: 일반담당교사',
  '합계: 교육받은교사',
  '합계: 강사',
] as const

const KNOWN_BUSINESS_AREAS = new Set<string>([
  '경제금융',
  '진로취업',
  '기업가정신',
  '디지털리터러시',
])

const TARGET_ORDER_FIVE = [
  'elementary',
  'middle',
  'high',
  'university',
  'adult',
] as const satisfies readonly TargetLevel[]

const TARGET_ORDER_DIGITAL = ['high', 'university', 'adult'] as const satisfies readonly TargetLevel[]

const TARGET_LABEL: Record<TargetLevel, string> = {
  elementary: '초',
  middle: '중',
  high: '고',
  university: '대학생',
  adult: '성인',
}

type PivotBucket = TargetLevel | 'unknown'

export type PivotMetrics = {
  schoolCount: number
  educationHours: number
  classCount: number
  male: number
  female: number
  totalParticipants: number
  generalVolunteers: number
  staffVolunteers: number
  returningVolunteers: number
  generalTeachers: number
  educatedTeachers: number
  instructors: number
}

const EMPTY_METRICS: PivotMetrics = {
  schoolCount: 0,
  educationHours: 0,
  classCount: 0,
  male: 0,
  female: 0,
  totalParticipants: 0,
  generalVolunteers: 0,
  staffVolunteers: 0,
  returningVolunteers: 0,
  generalTeachers: 0,
  educatedTeachers: 0,
  instructors: 0,
}

export type PivotExportRow =
  | { kind: 'category'; label: string }
  | { kind: 'detail'; label: string; metrics: PivotMetrics }
  | { kind: 'subtotal'; label: string; metrics: PivotMetrics }
  | { kind: 'grand'; label: string; metrics: PivotMetrics }

function sumClassCount(p: Program): number {
  return p.rounds.reduce((acc, r) => acc + (r.classCount ?? 0), 0)
}

function pivotBucket(p: Program): PivotBucket {
  const t = p.targetLevel
  if (t === 'elementary' || t === 'middle' || t === 'high' || t === 'university' || t === 'adult') {
    return t
  }
  return 'unknown'
}

function getCategoryLabel(p: Program): string {
  const raw = (p.businessArea ?? '').trim()
  if (KNOWN_BUSINESS_AREAS.has(raw)) return raw
  return '(비어 있음)'
}

function metricsFromPrograms(programs: Program[]): PivotMetrics {
  if (programs.length === 0) return { ...EMPTY_METRICS }

  const schoolKeys = new Set<string>()
  for (const p of programs) {
    schoolKeys.add(p.schoolId != null && p.schoolId !== '' ? String(p.schoolId) : `__row:${p.id}`)
  }

  return {
    schoolCount: schoolKeys.size,
    educationHours: programs.reduce((s, p) => s + (p.educationTime ?? 0), 0),
    classCount: programs.reduce((s, p) => s + sumClassCount(p), 0),
    male: programs.reduce((s, p) => s + (p.maleParticipants ?? 0), 0),
    female: programs.reduce((s, p) => s + (p.femaleParticipants ?? 0), 0),
    totalParticipants: programs.reduce((s, p) => s + (p.totalParticipants ?? 0), 0),
    generalVolunteers: programs.reduce((s, p) => s + (p.generalVolunteers ?? 0), 0),
    staffVolunteers: programs.reduce((s, p) => s + (p.staffVolunteers ?? 0), 0),
    returningVolunteers: programs.reduce((s, p) => s + (p.returningVolunteers ?? 0), 0),
    generalTeachers: programs.reduce((s, p) => s + (p.generalTeachers ?? 0), 0),
    educatedTeachers: programs.reduce((s, p) => s + (p.educatedTeachers ?? 0), 0),
    instructors: programs.reduce((s, p) => s + (p.instructors ?? 0), 0),
  }
}

/** 엑셀 B열부터 12개 숫자 (행 레이블 제외) */
export function metricsToNumericCells(m: PivotMetrics): number[] {
  return [
    m.schoolCount,
    m.educationHours,
    m.classCount,
    m.male,
    m.female,
    m.totalParticipants,
    m.generalVolunteers,
    m.staffVolunteers,
    m.returningVolunteers,
    m.generalTeachers,
    m.educatedTeachers,
    m.instructors,
  ]
}

const CATEGORY_ORDER: { label: string; detailTargets: readonly TargetLevel[] }[] = [
  { label: '경제금융', detailTargets: TARGET_ORDER_FIVE },
  { label: '진로취업', detailTargets: TARGET_ORDER_FIVE },
  { label: '기업가정신', detailTargets: TARGET_ORDER_FIVE },
  { label: '디지털리터러시', detailTargets: TARGET_ORDER_DIGITAL },
  { label: '(비어 있음)', detailTargets: TARGET_ORDER_FIVE },
]

/**
 * 필터된 실적 목록으로 피벗형 합계 시트 본문 행을 생성한다.
 */
export function buildPivotExportRows(programs: Program[]): PivotExportRow[] {
  const byCategory = new Map<string, Program[]>()
  for (const cat of CATEGORY_ORDER) {
    byCategory.set(cat.label, [])
  }
  for (const p of programs) {
    const label = getCategoryLabel(p)
    const bucket = byCategory.get(label)
    if (bucket) bucket.push(p)
    else {
      const fallback = byCategory.get('(비어 있음)')
      if (fallback) fallback.push(p)
    }
  }

  const rows: PivotExportRow[] = []

  for (const { label: catLabel, detailTargets } of CATEGORY_ORDER) {
    const inCategory = byCategory.get(catLabel) ?? []

    rows.push({ kind: 'category', label: catLabel })

    const byBucket = new Map<PivotBucket, Program[]>()
    for (const t of detailTargets) byBucket.set(t, [])
    byBucket.set('unknown', [])

    for (const p of inCategory) {
      const b = pivotBucket(p)
      if (b === 'unknown') {
        byBucket.get('unknown')!.push(p)
      } else if (detailTargets.includes(b as (typeof detailTargets)[number])) {
        byBucket.get(b)!.push(p)
      } else {
        byBucket.get('unknown')!.push(p)
      }
    }

    for (const t of detailTargets) {
      const list = byBucket.get(t) ?? []
      rows.push({
        kind: 'detail',
        label: `  ${TARGET_LABEL[t]}`,
        metrics: metricsFromPrograms(list),
      })
    }

    const unknownList = byBucket.get('unknown') ?? []
    if (unknownList.length > 0) {
      rows.push({
        kind: 'detail',
        label: '  (구분 없음)',
        metrics: metricsFromPrograms(unknownList),
      })
    }

    rows.push({
      kind: 'subtotal',
      label: '  합계',
      metrics: metricsFromPrograms(inCategory),
    })
  }

  rows.push({
    kind: 'grand',
    label: '총합계',
    metrics: metricsFromPrograms(programs),
  })

  return rows
}
