 ...
 * 시작·종료 중 있는 쪽만 기록 (열린 구간; 미러하지 않음).
 */
export function writeSponsorshipStartDateRangeToSearchParams(
  ...
  if (start) nextParams.set('sp_from', start.format('YYYY-MM-DD'))
  else nextParams.delete('sp_from')
  if (end) nextParams.set('sp_to', end.format('YYYY-MM-DD'))
  else nextParams.delete('sp_to')
}
