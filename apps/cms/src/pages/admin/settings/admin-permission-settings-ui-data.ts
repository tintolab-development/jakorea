...
export const ADMIN_PERMISSION_CATEGORIES: AdminPermissionCategoryDef[] = [
  {
    ...
    items: [
      ...
      { id: 'crud_notice_faq_inquiry_create', label: '공지사항 & FAQ & 문의 등록' },
      { id: 'crud_notice_faq_inquiry_delete', label: '공지사항 & FAQ & 문의 삭제' },
      { id: 'crud_notice_faq_inquiry_edit', label: '공지사항 & FAQ & 문의 수정' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'fv_program_participation', label: '프로그램 참여 이력' },
      { id: 'fv_settlement', label: '정산 내역' },
      { id: 'fv_student_assignment', label: '학생 과제' },
      { id: 'fv_edu_plan_diary', label: '교육계획서 및 교육일지' },
      { id: 'fv_lecture_report', label: '강의 보고서' },
      { id: 'fv_survey_and_result', label: '만족도조사 및 설문조사 결과' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'fd_edu_plan_diary', label: '교육계획서 및 교육일지' },
      { id: 'fd_lecture_report', label: '강의 보고서' },
      { id: 'fd_survey_and_result', label: '만족도조사 및 설문조사 결과' },
...
function createViewerFlags(): AdminPermissionFlags {
  ...
  const screenViewCategory = ADMIN_PERMISSION_CATEGORIES.find(
    category => category.id === 'screen_view'
  )
  if (!screenViewCategory) return viewerFlags

  const uncheckedSet = new Set<string>(VIEWER_FILE_VIEW_UNCHECKED_PERMISSION_IDS)
  for (const item of screenViewCategory.items) {
