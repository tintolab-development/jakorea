/**
 * 알림/발송 관리 API 연동용 OpenAPI 부분집합 생성
 * `openapi/backend.openapi.json` → `openapi/notifications.openapi.json`
 */
...
const outputPath = join(root, 'openapi/notifications.openapi.json')
...
const filteredPaths = Object.fromEntries(
  Object.entries(spec.paths ?? {}).filter(([path]) =>
    NOTIFICATIONS_PATH_PREFIXES.some(prefix => path.startsWith(prefix))
  )
)
...
const subset = {
  ...
  info: {
    ...spec.info,
    title: `${spec.info?.title ?? 'API'} — Notifications subset`,
    description: 'Filtered for CMS notifications / kakao alimtalk Orval codegen.',
  },
  paths: filteredPaths,
}
