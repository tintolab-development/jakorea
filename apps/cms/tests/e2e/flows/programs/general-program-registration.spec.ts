...
import { GeneralProgramRegistrationPage } from '../../pages/general-program-registration.page'
import { expectAuthenticatedShell } from '../../helpers/authenticated-shell'

/**
 * 저장된 어드민 세션 → 일반 프로그램 목록 직행 → 신규 등록 → 공통/모집/신청 작성
 * → 실 API 등록 완료 → 목록에서 작성 프로그램 확인
 *
 * 대시보드(`/`)를 거치지 않아 dashboard 위젯 병렬 타임아웃 폭주를 줄입니다.
 *
...
test.describe('일반 프로그램 신규 등록', () => {
  test('등록 완료 후 목록에 작성한 프로그램이 보인다', async ({ page }) => {
