...
- 풀페이지 내부 LNB/하위 LNB가 실제 화면 전환을 만든다면 breadcrumb depth에 포함한다.
  ...
  - 하위 LNB가 이미 리스트 맥락을 표현한다면 중첩 상세의 마지막 breadcrumb는 공통 상세명 없이 엔티티명만 표시한다.
    - 학교 상세: `초등학교`, `중학교` 등 학교명만 표시.
    - 강사 상세: 강사 이름만 표시.
    - 학생/지원자 상세: 학생·지원자 이름만 표시.
    - 기관/단체 상세: 기관명·단체명만 표시.
    - 예: `프로그램 목록 > 프로그램명 > 2차 면접 대상자 > 홍길동`
