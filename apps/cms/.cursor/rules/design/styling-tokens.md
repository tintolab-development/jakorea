...
- **디자인 토큰이 구현된 부분은 가장 우선적으로 사용한다.**  
  색상·간격·타이포·border-radius·shadow 등 `theme-provider.css`에 정의된 토큰이 있으면, 해당 값을 하드코딩하지 않고 반드시 `var(--토큰명)`으로 참조한다.
- 새 스타일을 작성할 때는 먼저 사용 가능한 토큰 목록을 확인하고, 토큰으로 표현할 수 있는 값은 모두 토큰을 사용한다.
