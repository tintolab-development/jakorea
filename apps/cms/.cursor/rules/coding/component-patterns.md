...
컴포넌트 작성 시 다음 원칙을 준수해야 합니다:

1. **재사용성**: 각 컴포넌트를 독립적으로 재사용 가능하게 작성
2. **테스트 용이성**: Hook과 컴포넌트를 각각 독립적으로 테스트 가능하게 작성
3. **유지보수성**: 관심사 분리로 수정 범위 최소화
4. **가독성**: 각 파일이 단일 책임을 가져 이해하기 쉽게 작성
5. **타입 안전성**: 모든 타입 체크 통과 (`pnpm typecheck`)

**적용 방법**:
- 컴포넌트가 200줄을 초과하면 분리 고려
- 비즈니스 로직은 Hook으로 분리
- UI 부분은 독립 컴포넌트로 추출
- 재사용 가능한 부분은 공통 컴포넌트로 분리
