...
CMS **대시보드 홈 → 대시보드 설정** 모달·바로가기 위젯을 FE mock과 동일하게 검증할 수 있도록, 바로가기 카탈로그 24건 + 로컬 데모 관리자 preferences 1건(역할당)을 시드합니다.
...
- 카탈로그: `dashboard_shortcut` (바로가기 마스터, `use_yn=true` 24건. 메일 발송 이력·회원 로그인 이력은 기획 제외, `admin-permission-settings` 포함)
...
| `shortcutVisibility` | `SHORTCUT_ITEMS` 24개 id 전부 `true` |
...
V42에 있던 `email-history`·`member-login-history`는 기획에서 제외합니다 (`use_yn=false` 또는 삭제). 아래는 FE mock과 맞춰 주세요.
...
1. 로컬 데모 MASTER 로그인 후 `GET /api/admin/dashboard/shortcuts` → 24건, `admin-permission-settings` 포함, 메일/로그인 이력 제외.
2. `GET /api/admin/me/dashboard-preferences` → `assignedProgramTypes` 포함, `shortcutVisibility` 24키 전부 true, `widgetProgramFilters` 빈 객체(또는 키 없음).
3. CMS `/` 「대시보드 설정」: 바로가기 24개 + 전체 선택, 위젯별 프로그램 아코디언이 잘리지 않고 스크롤된다.
