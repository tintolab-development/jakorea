...
CMS **대시보드 홈 → 대시보드 설정** 모달·바로가기 위젯을 FE mock과 동일하게 검증할 수 있도록, 바로가기 카탈로그 **26건**(`email-history`·`member-login-history` 포함) + 로컬 데모 관리자 preferences 1건(역할당)을 시드합니다.
...
- 카탈로그: `dashboard_shortcut` (바로가기 마스터, `use_yn=true` **26건**. `email-history`·`member-login-history`·`admin-permission-settings` 포함)
...
| `shortcutVisibility` | `SHORTCUT_ITEMS` 26개 id 전부 `true` (`email-history`·`member-login-history` 포함) |
...
MASTER는 담당 유형이 전체이므로 일정 위젯 4종을 기본 레이아웃에 넣습니다 (`getProgramScheduleKindsForAdminUser`). `email-history`·`member-login-history`는 members/api 바로가기 SSOT이므로 카탈로그에 유지합니다.

---

## 2. 카탈로그 갭 (현재 BE vs mock)

V42는 아래 건을 mock과 다르게 둡니다. **use_yn=true** 로 맞추고 라벨·path를 FE와 동일하게 하세요.

| id | shortcutKey | mock 라벨 | mock path |
|----|-------------|-----------|-----------|
| 162921 | `email-history` | 메일 발송 이력 | `/` |
| 162925 | `member-login-history` | 회원 로그인 이력 | `/logs/member-login-history` |
| 162926 | `admin-permission-settings` | 관리자 권한 설정 | `/admin/settings/permissions` |
...
1. 로컬 데모 MASTER 로그인 후 `GET /api/admin/dashboard/shortcuts` → 26건, `email-history`·`member-login-history`·`admin-permission-settings`·로그 이력 포함.
2. `GET /api/admin/me/dashboard-preferences` → `assignedProgramTypes` 포함, `shortcutVisibility` 26키 전부 true, `widgetProgramFilters` 빈 객체(또는 키 없음).
3. CMS `/` 「대시보드 설정」: 바로가기 26개 + 전체 선택, 위젯별 프로그램 아코디언이 잘리지 않고 스크롤된다.
