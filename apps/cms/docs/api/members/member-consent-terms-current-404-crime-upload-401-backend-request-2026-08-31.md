...
CMS에서 동의서 작성형 항목을 한 건 이상 **동의·제출**한 뒤 흐름이 끊깁니다.
...
공통 원인 후보는 **약관 문서 카탈로그 키(`PAYMENT_STATEMENT_PRE_CONSENT`)와 회원 원장 키(`PAYMENT_STATEMENT_CONSENT`) 불일치**, 그리고 **회원 생성 전 성범죄 파일 업로드 인증/owner 계약**입니다.
...
3. 동의서 작성 필요 항목 중 **1건 이상** 작성 후 **동의**로 제출.
   - 지급조서만이 아님. 초상권·지급조서·교육진행자·행정정보·성범죄 중 하나여도 재현됨.
...
1. 등록 시 FE는 지급조서를 **`PAYMENT_STATEMENT_PRE_CONSENT`** 로 보냄 (`build-pre-register-terms-agreements.ts`).
2. OpenAPI `TermsAgreementRequest.termsType` 설명:  
   **`PAYMENT_STATEMENT_PRE_CONSENT` 입력은 회원 원장의 `PAYMENT_STATEMENT_CONSENT`로 정규화**.
3. 상세 GET `termsAgreements[]`에는 정규화된 **`PAYMENT_STATEMENT_CONSENT`** 가 내려옴 (동의 여부와 무관하게 행이 있으면).
4. 상세 수정 draft는 GET 행을 그대로 복사 (`termsAgreementRowsToRequests`).
5. 저장 직전 `resolvePreRegisterTermsAgreementVersions`가 각 `termsType`에 대해  
   `GET /api/public/terms-documents/{termsType}/current` 를 호출.
...
2. 상세 GET의 `termsType`과 current path `termsType`을 **한 세트로 맞출 것**.
...
4. 없는 타입만 `TERMS_DOCUMENT_NOT_FOUND`. 정규화 별칭은 200.
...
회원은 아직 없음. `memberId` 없이 업로드한다.

### 3.2 FE 호출 순서

`upload-consent-evidence-file.ts` · 선행 문서 §5.2와 동일.
...
Network에서 **어느 URL이 401인지** 구분해 달라. 후보는 아래.
...
화면에서 401이 보이면 대개 **B 또는 A를 Network에 남긴 것**.
...
- 성범죄 업로드를 “회원 생성 후”로 미뤄 등록을 막지 말 것. 동의서 작성형 「동의」는 **작성·업로드 완료 후**만 인정 ([terms-and-consent-policy](../../../../.cursor/rules/terms-and-consent-policy.mdc)).
...
| 파일 | 역할 |
|------|------|
| `apps/cms/src/features/user/api/fetch-current-terms-document.ts` | `GET .../terms-documents/{type}/current` |
| `apps/cms/src/features/user/api/resolve-pre-register-terms-agreement-versions.ts` | 등록·상세 PATCH 직전 version 갱신 |
| `apps/cms/src/features/user/api/member-basic-info-terms-patch.ts` | GET 행 → PATCH draft (`termsType` 유지) |
| `apps/cms/src/features/user/api/upload-consent-evidence-file.ts` | 성범죄 prepare → PUT → confirm |
