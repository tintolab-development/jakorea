...
|---|------|----------|---------|------|---------|------|
| 6 | 지급조서 확인 상세·목록 / 회원·교사 정산 탭 | `/settlement-management/payment-orders?…`, 회원 상세 | `payment-order-detail-filter-table.tsx`, `payment-order-*-settlement-table.tsx`, `instructor-payment-tab.tsx`, `teacher-settlement-tab.tsx` | `지급조서 발급` | **클라이언트 PDF** — 단건 미리보기·벌크 순차 fallback. DTO 없으면 **공란**(목 샘플 없음). ZIP API·원문 PII는 BE 대기 | 선택 라인 **지급조서 bulk ZIP API 신규 (SET-006)**. **PDF 본문은 마스킹 해제 원문 (SET-009)** — [UI SSOT §1.2](./settlement-payment-order-detail-ui-fields-backend-handoff.md#12--p0-서버-요청--지급조서-발급원문--산출-내역서-unmask-api) |
