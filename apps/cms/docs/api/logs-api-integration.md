...
- **회원 로그인 이력만 예외:** remote 실패·미로그인 시 mock(`data/mock/member-login-logs.ts`)을 페이지 단위로 표시합니다.
  - 기획: 수집일로부터 1개월 보관 후 파기 — 요청 `from`을 cutoff로 clamp (mock는 조회 후 `filterMemberLoginLogsByRetention`)
