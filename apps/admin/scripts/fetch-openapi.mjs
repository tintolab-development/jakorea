/**
 * OpenAPI 스냅샷 다운로드
 * `openapi/backend.openapi.json` ← Homepage Admin `/v3/api-docs`
 *
 * 우선순위: `VITE_HOMEPAGE_API_SERVER` → `VITE_API_SERVER` → `http://localhost:8081`
 * (런타임 로그인용 `VITE_API_SERVER`는 CMS 백엔드일 수 있으므로 Homepage 전용 env를 권장)
 */
...
if (!response.ok) {
  throw new Error(
    `OpenAPI fetch failed: HTTP ${response.status} ${response.statusText}\n` +
      `URL: ${url}\n` +
      'Homepage Admin 백엔드 서버/ngrok URL을 확인하거나 git에 커밋된 openapi/backend.openapi.json을 사용하세요.',
  )
}
