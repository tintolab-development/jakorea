/**
 * JA_ORGANIZATION_CHART_IMAGE asset prepare → binary upload → confirm
 */
...
export async function uploadOrgChartImageAsset(file: File): Promise<number> {
  const assetsApi = getJAKoreaHomepageAdminAPIAssetsSubset()
  const prepared = (await assetsApi.prepareUpload({
    purpose: 'JA_ORGANIZATION_CHART_IMAGE',
  ...
  if (assetId == null || version == null || !uploadUrl) {
    throw new Error('조직도 이미지 업로드 준비에 실패했습니다.')
  }
  ...
  if (!uploadRes.ok) {
    throw new Error(`조직도 이미지 업로드에 실패했습니다. (${uploadRes.status})`)
  }
