...
import {
  exportFileDownloadLogsService,
  listFileDownloadLogsService,
} from './service'
...
export function useExportFileDownloadLogs() {
  return useMutation({
    mutationFn: (filter: FileDownloadListFilter) =>
      exportFileDownloadLogsService(filter),
    retry: false,
  })
}
