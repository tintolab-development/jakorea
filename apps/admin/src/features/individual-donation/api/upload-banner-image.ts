/**
 * SPONSORSHIP_BANNER_IMAGE asset prepare → binary upload → confirm
 */
...
export async function uploadSponsorshipBannerAsset(file: File): Promise<number> {
  const assetsApi = getJAKoreaHomepageAdminAPIAssetsSubset()
  const prepared = (await assetsApi.prepareUpload({
    purpose: 'SPONSORSHIP_BANNER_IMAGE',
