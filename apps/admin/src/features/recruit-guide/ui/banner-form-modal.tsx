...
const IMAGE_GUIDE_LINES = [
  ...
  '- 권장 배너 사이즈는 740*790입니다.',
]

const MODAL_DESCRIPTION = '채용 페이지의 상단 배너 이미지와 문구를 설정해 주세요.'
...
function BannerFormBody({ initial, confirmLoading, onCancel, onSubmit }: FormBodyProps) {
  ...
  const [subText01, setSubText01] = useState(initial.subText01)
  const [subText02, setSubText02] = useState(initial.subText02)
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!subText01.trim()) {
      nextErrors.subText01 = '서브 텍스트를 입력해 주세요.'
    }
    if (!subText02.trim()) {
      nextErrors.subText02 = '서브 텍스트를 입력해 주세요.'
    }
  ...
  return (
    <ContentModal
      ...
      width={800}
      className="recruit-guide-banner-modal"
      footer={
        <>
          <CmsButton
            variant="secondary"
    ...
    >
      <DetailInfoForm
        ...
        hideHeader
        mode="edit"
        className="recruit-guide-banner-modal__form"
      >
        <DetailInfoForm.Row type="single">
          <DetailInfoForm.Field
            ...
            edit={
              <div>
                <div className="recruit-guide-banner-modal__image-row">
                  <div className="recruit-guide-banner-modal__preview">
                    {imageUrl ? (
                  ...
                  <FileSelectField
                    className="recruit-guide-banner-modal__file-field"
                    multiple={false}
                    accept={IMAGE_ACCEPT}
                    buttonLabel="파일 추가"
                ...
                {errors.image ? (
                  <p className="recruit-guide-banner-modal__field-error" role="alert">
                    {errors.image}
                  </p>
                ) : null}
              </div>
            ...
            edit={
              <div>
                <CmsTextArea
                  ...
                  value={subText01}
                  onChange={e => {
                    setSubText01(e.target.value)
                    if (e.target.value.trim()) {
                      setErrors(prev => ({ ...prev, subText01: undefined }))
                    }
                  }}
                />
                {errors.subText01 ? (
            ...
            edit={
              <div>
                <CmsTextArea
                  ...
                  value={subText02}
                  onChange={e => {
                    setSubText02(e.target.value)
                    if (e.target.value.trim()) {
                      setErrors(prev => ({ ...prev, subText02: undefined }))
                    }
                  }}
                />
                {errors.subText02 ? (
...
export function BannerFormModal({
  ...
  return (
    <BannerFormBody
      key={`${initial.imageUrl}-${initial.mainText}-${initial.subText01}`}
      initial={initial}
