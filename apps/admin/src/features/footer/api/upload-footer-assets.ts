/**
 * FOOTER_LOGO / FOOTER_PARTNER_LOGO asset prepare → binary upload → confirm
 */
...
type FooterAssetPurpose = Extract<
  UploadPrepareRequestPurpose,
  'FOOTER_LOGO' | 'FOOTER_PARTNER_LOGO'
>

async function uploadFooterAsset(file: File, purpose: FooterAssetPurpose): Promise<number> {
...
export function uploadFooterLogoAsset(file: File): Promise<number> {
  return uploadFooterAsset(file, 'FOOTER_LOGO')
}

export function uploadFooterPartnerLogoAsset(file: File): Promise<number> {
  return uploadFooterAsset(file, 'FOOTER_PARTNER_LOGO')
}
